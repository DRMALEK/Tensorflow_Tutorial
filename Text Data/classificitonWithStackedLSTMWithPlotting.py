...
model = tf.keras.Sequential([
    ...
    # Rule of thump : embedding dim == LSTM units count
    tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(
        16), return_sequences=True, dropout=0.2),
...
history = model.fit(x=np.array(training_padded),
                    ...
                    validation_data=(np.array(testing_padded),
...
Plot.plot_history_line('Epochs', 'Accuracy', train_acc, 'accuracy',
                       val_acc, 'val_accuracy', figure_name='stackedLastmAccuracy.png')

# Plot the train vs val loss chart
Plot.plot_history_line('Epochs', 'Loss', train_loss, 'loss',
                       val_loss, 'val_loss', figure_name='stackedLstmLoss.png')
